...
class NumberTests: XCTestCase {
    func testNumbers() {
        let phi_hat:Double = (1 + sqrt(5.0)) / 2.0
        // tests close
        XCTAssertTrue(close(phi, y: phi_hat))
        XCTAssertTrue(close(0, y: 1e-10) == true)
        XCTAssertTrue(rad2deg(pi/2) == 90)
        XCTAssertTrue(deg2rad(90) == pi/2)
        XCTAssertTrue(max(0, 1) == 1)
        XCTAssertTrue(min(0, 1) == 0)
        XCTAssertTrue(3 / 4 == 0.75)
        XCTAssertTrue(3.25 / 4 == 0.8125)
        XCTAssertTrue(isNumber(3))
